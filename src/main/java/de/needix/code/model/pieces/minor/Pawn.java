    ...
    public List<Point> getValidMoves(Board currentBoard, Point currentPosition) {
        ...
        switch (getTeam()) {
            case WHITE:
                ...
                int newYLeft = currentPosition.y - 1;
                int newYRight = currentPosition.y + 1;

                Point posForward = new Point(newX, currentPosition.y);
                ...
                if (currentBoard.isInBounds(posForward)) {
                    Piece frontPiece = currentBoard.getPiece(newX, currentPosition.y);
                    if (!frontPiece.isValidPiece()) {
                        validMoves.add(new Point(newX, currentPosition.y));
                    }
                } else if (currentBoard.isInBounds(posForwardLeft)) {
                    Piece frontLeftPiece = currentBoard.getPiece(newX, newYLeft);
                    if (frontLeftPiece.isValidPiece()
                            && frontLeftPiece.getTeam() != this.getTeam()) {
                    ...
                    if (frontRightPiece.isValidPiece()
                            && frontRightPiece.getTeam() != this.getTeam()) {
